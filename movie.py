...
def download_file_from_google_drive(id, destination):
    """Fallback downloader for Google Drive large files."""
...
def ensure_file(file_id, filename):
    """Ensure file exists locally. Try gdown first, then requests fallback."""
...
if not (ok1 and ok2):
    st.error("❌ Failed to load required data files. Please check Drive sharing and IDs.")
    st.stop()
